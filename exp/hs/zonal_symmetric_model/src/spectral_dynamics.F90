...
real, allocatable, dimension(:,:,:) :: du_dt, dv_dt, dt_dt
real, allocatable, dimension(:,:,:) :: du_dt_m, dv_dt_m, dt_dt_m
real, allocatable, dimension(:,:)   :: dln_ps_dt, dln_ps_dt_m
...
real    :: damping_coeff       = 1.15740741e-4, & ! (one tenth day)**-1
 ...
 character(len=128) :: forcing='', tendency=''   
!===============================================================================================
...
if(trim(forcing)/='') then
  if(mpp_pe() == mpp_root_pe()) print*,trim(forcing)
  if(file_exist(trim(forcing))) then
    allocate (du_dt(is:ie, js:je, num_levels))
    allocate (dv_dt(is:ie, js:je, num_levels))
    allocate (dt_dt(is:ie, js:je, num_levels))
    allocate (dln_ps_dt(is:ie, js:je))
    call read_data(trim(forcing),'du_dt',du_dt(is:ie, js:je,:))
    call read_data(trim(forcing),'dv_dt',dv_dt(is:ie, js:je,:))
    call read_data(trim(forcing),'dt_dt',dt_dt(is:ie, js:je,:))
    call read_data(trim(forcing),'dln_ps_dt',dln_ps_dt(is:ie, js:je))
  else
    call error_mesg('spectral_dynamics_init',trim(forcing)//' is not found!', FATAL)
  endif  
endif
...
if(trim(forcing)/='') then
   dt_ug_tmp  = dt_ug_tmp +du_dt
   dt_vg_tmp  = dt_vg_tmp +dv_dt
endif
...
!----------------------------------------------------------------------------------------------
if(trim(tendency)/='') then
  call set_domain(grid_domain)
  if(mpp_pe() == mpp_root_pe()) print*,trim(tendency)  
    allocate (du_dt_m(is:ie, js:je, num_levels))
    allocate (dv_dt_m(is:ie, js:je, num_levels))
    allocate (dt_dt_m(is:ie, js:je, num_levels))
    allocate (dln_ps_dt_m(is:ie, js:je))
    
    call uv_grid_from_vor_div(dt_vors, dt_divs, du_dt_m, dv_dt_m)
    call trans_spherical_to_grid(dt_ts, dt_dt_m)
    call trans_spherical_to_grid(dt_ln_ps, dln_ps_dt_m)
    
    call write_data(trim(tendency),'du_dt',-du_dt_m(is:ie, js:je,:))
    call write_data(trim(tendency),'dv_dt',-dv_dt_m(is:ie, js:je,:))
    call write_data(trim(tendency),'dt_dt',-dt_dt_m(is:ie, js:je,:))
    call write_data(trim(tendency),'dln_ps_dt',-dln_ps_dt_m(is:ie, js:je))
endif
!----------------------------------------------------------------------------------------------
