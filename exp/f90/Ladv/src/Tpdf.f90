...
!--------------------------------------------------------------------------
 real,    allocatable, dimension(:,:):: ug, vg, theta_eq, forcing, ug_mean
...
!initialization of q tracers and damping
 ...
 do j=1, num_lat
   ...
   ug_mean (i,j)= 8*(cos(2.*lon(i)*pi/180.)-0.4)
 end do
 end do
...
!	   print*, r1, r2
        ...
        call compute_tracer_advection(ug+ug_mean, vg, q_tracer, qs_tracer)
...
!    dt_vorg(i,j) = -angular_speed*cos(lat(j)*pi/180.)*dvorg_dx(i,j) -v(i,j)*beta(j) -ks*vorg(i,j,previous) +forcing(i,j)
    dt_vorg(i,j) = -angular_speed*cos(lat(j)*pi/180.)*dvorg_dx(i,j) -ks*vorg(i,j,previous) +forcing(i,j)
 end do
 end do
