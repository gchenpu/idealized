...
use                 topography_mod, only: gaussian_topog_init, sine_topog_init, get_topog_mean, get_ocean_mask
...
!  then immediately loaded back into surf_geopotential
   ...
   call get_deg_lon(deg_lon)
   call get_deg_lat(deg_lat)
   call sine_topog_init(deg_lon*pi/180, deg_lat*pi/180, surf_height)
   surf_geopotential = grav*surf_height
else 
