...
integer :: id_ps, id_u, id_v, id_t, id_vor, id_div, id_omega, id_wspd, id_slp, id_u_adv, id_v_adv, id_w_adv
...
character(len=64) :: advecting_flow = ''
real :: advecting_ratio = 0.
!===============================================================================================
...
namelist /spectral_dynamics_nml/ use_virtual_temperature, damping_option,                            &
                                 ...
                                 advecting_flow, advecting_ratio
...
integer :: j, k, time_level, seconds, days, n, i
real    :: delta_t, value, surface_value, ys, ye, time_in_days, onset, shutdown, p_b
character(len=256) :: scheme, params
...
if(trim(advecting_flow)=='') then
  call update_tracers(tracer_attributes, dt_tracers_tmp, wg, p_half, delta_t)
else
  call update_tracers_eddy(tracer_attributes, dt_tracers_tmp, wg, p_half, delta_t)
endif
...
do n=1,num_tracers
    if (query_method('tracer_boundary_condition', MODEL_ATMOS, n, scheme, params)) then
          if (parse(params,'surface_value',value) == 1) then
	      surface_value = value
          else
	      surface_value = 0.
	  endif
	  ...
	  if (parse(params,'shutdown',value) == 1) then
	      shutdown = value
          else
	      shutdown = 0.
	  endif
	  if (parse(params,'ys',value) == 1) then
	      ys = value
          else
	      ys = 30.
	  endif
	  if (parse(params,'ye',value) == 1) then
	      ye = value
          else
	      ye = 50.
	  endif
	  if (parse(params,'p_b',value) == 1) then
	      p_b = value
          else
	      p_b = 0.8e5
	  endif
       if (uppercase(trim(scheme)) == 'FIXED') then
          if(time_in_days >= onset) then
             do k=1, num_levels
	     do i=is, ie
             do j=js, je
  	        if ((lat(j)>ys .and. lat(j)<=ye .and. p_full(i,j,k)>p_b)) grid_tracers(i,j,k,future,n) = surface_value
             end do
	     end do
	     end do
             if(trim(tracer_attributes(n)%numerical_representation) == 'spectral') &
  	            call trans_grid_to_spherical(grid_tracers(:,:,:,future,n), spec_tracers(:,:,:,future,n))
          endif
       else if (uppercase(trim(scheme)) == 'PULSE') then
          if(time_in_days >= onset .and. (shutdown ==0. .or. time_in_days < shutdown)) then
             do k=1, num_levels
	     do i=is, ie
             do j=js, je
  	        if ((lat(j)>ys .and. lat(j)<=ye .and. p_full(i,j,k)>p_b)) grid_tracers(i,j,k,future,n) = surface_value
             ...
             if(trim(tracer_attributes(n)%numerical_representation) == 'spectral') &
  	            call trans_grid_to_spherical(grid_tracers(:,:,:,future,n), spec_tracers(:,:,:,future,n))
          else if(time_in_days >= shutdown) then
             do k=1, num_levels
	     do i=is, ie
             do j=js, je
  	        if ((lat(j)>ys .and. lat(j)<=ye .and. p_full(i,j,k)>p_b)) grid_tracers(i,j,k,future,n) = 0.
             end do
	     end do
	     end do
             if(trim(tracer_attributes(n)%numerical_representation) == 'spectral') &
...
subroutine update_tracers_eddy(tracer_attributes, dt_tr, wg, p_half, delta_t)
...
complex, dimension(ms:me, ns:ne, num_levels) :: dt_trs
real,    dimension(is:ie, js:je, num_levels) :: dp, dt_tmp, tr_future, ug_new, vg_new
real,    dimension(is:ie, js:je, num_levels+1) :: wg_new
integer :: ntr, time_level
logical :: used

dp = p_half(:,:,2:num_levels+1) - p_half(:,:,1:num_levels)

call get_advecting_flow(ug_new, vg_new, wg_new)

do ntr = 1, num_tracers
  if(trim(tracer_attributes(ntr)%numerical_representation) == 'spectral') then
    call horizontal_advection(spec_tracers(:,:,:,current,ntr), ug_new, vg_new, dt_tr(:,:,:,ntr))
    if(tracer_vert_advect_scheme(ntr) == SECOND_CENTERED .or. &
       tracer_vert_advect_scheme(ntr) == FOURTH_CENTERED)         time_level=current
    if(tracer_vert_advect_scheme(ntr) == VAN_LEER_LINEAR .or. &
       tracer_vert_advect_scheme(ntr) == FINITE_VOLUME_PARABOLIC) time_level=previous
    call vert_advection(delta_t, wg_new, dp, grid_tracers(:,:,:,time_level,ntr), dt_tmp, &
                     scheme=tracer_vert_advect_scheme(ntr), form=ADVECTIVE_FORM)
    dt_tr(:,:,:,ntr) = dt_tr(:,:,:,ntr) + dt_tmp
    if(trim(tracer_attributes(ntr)%hole_filling) == 'on') then
      call water_borrowing (dt_tr(:,:,:,ntr), grid_tracers(:,:,:,previous,ntr), current, p_half, delta_t)
    ...
    tr_future = grid_tracers(:,:,:,previous,ntr) + delta_t*dt_tr(:,:,:,ntr)
    dt_tr(:,:,:,ntr) = 0.0
    call a_grid_horiz_advection (ug_new, vg_new, tr_future, delta_t, dt_tr(:,:,:,ntr))
    tr_future = tr_future + delta_t*dt_tr(:,:,:,ntr)
    dp = p_half(:,:,2:num_levels+1) - p_half(:,:,1:num_levels)
    call vert_advection(delta_t, wg_new, dp, tr_future, dt_tmp, scheme=tracer_vert_advect_scheme(ntr), form=ADVECTIVE_FORM)
    tr_future = tr_future + delta_t*dt_tmp
    if(step_number == num_steps) then
      grid_tracers(:,:,:,current,ntr) = grid_tracers(:,:,:,current,ntr) + &
      tracer_attributes(ntr)%robert_coeff*(grid_tracers(:,:,:,previous,ntr) - 2.0*grid_tracers(:,:,:,current,ntr))
...
return
end subroutine update_tracers_eddy
...
!===================================================================================
subroutine spectral_diagnostics(Time, p_surf, u_grid, v_grid, t_grid, wg_full, tr_grid, time_level, nhum)
...
integer, intent(in), optional :: nhum

real, dimension(is:ie, js:je, num_levels)    :: ln_p_full, p_full, z_full, worka3d, workb3d, work, work1
real, dimension(is:ie, js:je, num_levels+1)  :: ln_p_half, p_half, z_half, work2
...
if(id_u_adv > 0 .or. id_v_adv > 0 .or. id_w_adv > 0) then
  call get_advecting_flow(work, work1, work2)
  if(id_u_adv   > 0)    used = send_data(id_u_adv, work, Time)
  if(id_v_adv   > 0)    used = send_data(id_v_adv, work1, Time)
  if(id_w_adv   > 0)    used = send_data(id_w_adv, work2, Time)
endif
...
!===================================================================================
subroutine get_advecting_flow(u_new, v_new, w_new)
real, intent(out), dimension(is:ie, js:je, num_levels) :: u_new, v_new
real, intent(out), dimension(is:ie, js:je, num_levels+1) :: w_new

real, dimension(js:je) :: lat
real, dimension(num_levels) :: p_full_gm, theta_z0, dtheta_z0_dp, theta_gm, dtheta_gm_dp
real, dimension(is:ie, js:je ) :: dx_psg, dy_psg, dp, dmean, dmean_tot, psg_zm
real, dimension(is:ie, js:je, num_levels) :: theta, dens, divg_new, vorg_new
real, dimension(js:je, num_levels) :: v_zm, w_zm, theta_zm, dtheta_zm_dp, chi, dchi_dp, dp_zm, pfull_zm, evth_zm

real, dimension(is:ie, js:je, num_levels)   :: p_full, ln_p_full
real, dimension(is:ie, js:je, num_levels+1) :: p_half, ln_p_half
real, dimension(is:ie, 1:lat_max, num_levels) :: p_full_global
complex,dimension(ms:me, ns:ne) :: ln_ps_zm
complex,dimension(ms:me, ns:ne, num_levels) :: vors_new, divs_new
integer :: i, j, k

if(trim(advecting_flow)=='zonal_mean') then
! v*dp is zonally symmetric
  do k=1, num_levels
    dp(:,:) = dpk(k) + dbk(k)*psg(:,:,current)
    v_zm(:,k) = sum(dp(:,:)*vg(:,:,k,current),1)/size(dp,1)
    do i=is, ie
      v_new(i,:,k) = v_zm(:,k)/dp(i,:)
    end do
  end do
  u_new = 0
  
else if(trim(advecting_flow)=='residual_mean') then  
! residual mean velocity
  call pressure_variables(p_half, ln_p_half, p_full, ln_p_full, psg(:,:,current))
  theta = tg(:,:,:,current)*((1e5/p_full(:,:,:))**KAPPA)
  
  do k=1, num_levels
    p_full_gm(k) = area_weighted_global_mean(p_full(:,:,k))
    theta_gm(k)  = area_weighted_global_mean(theta(:,:,k))
  end do
  dtheta_gm_dp = diff(theta_gm,p_full_gm)

  do k=1, num_levels
    dp(:,:) = dpk(k) + dbk(k)*psg(:,:,current)
    v_zm(:,k) = sum(dp(:,:)*vg(:,:,k,current),1)/size(dp,1)
    theta_zm(:,k) = sum(theta(:,:,k),1)/size(theta,1)
    do j=js, je
      chi(j,k) = sum((dp(:,j)*vg(:,j,k,current)-v_zm(j,k))*(theta(:,j,k)-theta_zm(j,k)),1)/size(dp,1)/dtheta_gm_dp(k)
    end do 
  end do
  
  do j=js,je
    dchi_dp(j,:) = diff(chi(j,:),p_full_gm)
  end do
  
  do k=1, num_levels
    dp(:,:) = dpk(k) + dbk(k)*psg(:,:,current)
    do i=is, ie
      v_new(i,:,k) = (v_zm(:,k) - dchi_dp(:,k))/dp(i,:)
    end do
  end do
  u_new = advecting_ratio*ug(:,:,:,current)
  v_new = v_new + advecting_ratio*(vg(:,:,:,current)-v_new)
  
else if(trim(advecting_flow)=='residual_eddy') then  
! residual eddy flux
  call pressure_variables(p_half, ln_p_half, p_full, ln_p_full, psg(:,:,current))
  theta = tg(:,:,:,current)*((1e5/p_full(:,:,:))**KAPPA)
  ...
  u_new = ug(:,:,:,current)
  v_new = (vg(:,:,:,current)-v_new) + advecting_ratio*v_new
...
! obtain the vertical mass flux from the continuity equation as in subroutine four_in_one using 'simmons_and_burridge' option
  call vor_div_from_uv_grid(u_new, v_new, vors_new, divs_new, triang = triang_trunc)
  call trans_spherical_to_grid(divs_new, divg_new)
  call compute_pressure_gradient(ln_ps(:,:,current), psg(:,:,current), dx_psg, dy_psg)
  
  dmean_tot = 0.
  do k = 1,num_levels
    dp = dpk(k) + dbk(k)*psg(:,:,current)
    dmean = divg_new(:,:,k)*dp + dbk(k)*(u_new(:,:,k)*dx_psg+v_new(:,:,k)*dy_psg)
    dmean_tot = dmean_tot + dmean
    w_new(:,:,k+1) = - dmean_tot
  enddo
  
! For a zonally symmetric tracer, this ensures the solution is zonally symmetric
!  do k = 2,num_levels
!    w_new(:,:,k) = w_new(:,:,k) + dmean_tot*bk(k)
!  enddo

  w_new(:,:,1           ) = 0.0
  w_new(:,:,num_levels+1) = 0.0

end subroutine get_advecting_flow
!===================================================================================
function diff(y,x) result(dy_dx)
real,intent(in), dimension(:)::y,x
real,dimension(size(y))::dy_dx
integer :: i, ni
ni=size(y);
