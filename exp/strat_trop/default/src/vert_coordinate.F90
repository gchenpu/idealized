...
else if(trim(vert_coord_option) == 'uneven_sigma_new') then
  call compute_uneven_sigma_new(a, b)
...
subroutine compute_uneven_sigma_new (a, b)

real,    intent (out), dimension(:) :: a, b
real :: scale_heights, surf_res, exponent

integer :: k, num_levels
real :: z, zeta, s2, sig_top=1e-5, sig_half=0.1

num_levels = size(a,1)-1

a = 0.0

scale_heights = -log(sig_top)

exponent=log(-log(sig_half)/scale_heights)/log(0.5)
...
return
end subroutine compute_uneven_sigma_new
