...
public :: gaussian_topog_init, sine_topog_init, get_gaussian_topog, get_sine_topog
...
!    </DATA>
   ...
   real, dimension(maxmts) :: topo_height = 0.
   real, dimension(maxmts) ::  topo_lat1  = 0.
   real, dimension(maxmts) ::  topo_lat2  = 0.
   real, dimension(maxmts) ::  topo_wnum  = 0.
   real, dimension(maxmts) ::  topo_lon0  = 0.
   real, dimension(maxmts) ::  topo_tilt  = 0.
!   namelist /gaussian_topog_nml/ height, olon, olat, wlon, wlat, rlon, rlat
   namelist /gaussian_topog_nml/ height, olon, olat, wlon, wlat, rlon, rlat, &
                                 topo_height, topo_lat1, topo_lat2, topo_wnum, topo_lon0, topo_tilt
! </NAMELIST>
...
!#######################################################################
subroutine sine_topog_init ( lon, lat, zsurf )
...
integer :: n
  ...
  if(any(shape(zsurf) /= (/size(lon(:)),size(lat(:))/))) then
    call error_mesg ('get_sine_topog in topography_mod', &
     'shape(zsurf) is not equal to (/size(lon),size(lat)/)', FATAL)
  endif
...
! compute sum of all non-zero mountains
  zsurf(:,:) = 0.
  do n = 1, maxmts
    if ( topo_height(n) == 0. ) cycle
    zsurf = zsurf + get_sine_topog ( lon, lat, topo_height(n), &
               topo_lat1(n), topo_lat2(n), topo_wnum(n), topo_lon0(n),topo_tilt(n))
  enddo
 module_is_initialized = .TRUE.                    

end subroutine sine_topog_init
...
!#######################################################################
function get_sine_topog ( lon, lat, height,                          &
                              lat1d, lat2d, wnum0, lon0d,topo_tilt ) &
                       result ( zsurf )

real, intent(in)  :: lon(:), lat(:)
real, intent(in)  :: height
real, intent(in), optional :: lat1d, lat2d, wnum0, lon0d, topo_tilt
real :: zsurf(size(lon,1),size(lat,1))

integer :: i, j
real    :: lat1, lat2, wnum, lon0, lat0, topo_center
real    :: tpi, dtr, dy, wy
...
! defaults and convert degrees to radians (dtr)
  lat1 = 30.*dtr;  if (present(lat1d)) lat1=lat1d*dtr
  lat2 = 60.*dtr;  if (present(lat2d)) lat2=lat2d*dtr
  lon0 =  0.*dtr;  if (present(lon0d)) lon0=lon0d*dtr
  wnum =  0.;      if (present(wnum0)) wnum=wnum0

! compute sine mountain
  wy = lat2 - lat1
  topo_center=(lat2-lat1)/2+lat1;
  zsurf(:,:) = 0.
  do j=1,size(lat(:))
    dy = max(0.,lat(j)-lat1)
    if ( dy.gt.wy ) dy = wy
    do i=1,size(lon(:))
      zsurf(i,j) = height*( sin( pi*dy/wy )**2 )  &
                         *cos( wnum*(lon(i) - (lon0+(lat(j)-topo_center)*topo_tilt)   ))
    enddo
  enddo

end function get_sine_topog
