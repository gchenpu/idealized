...
use          tracer_type_mod, only: tracer_type, tracer_type_init

use   tracer_source_sink_mod, only: radon_source_sink
...
'$Id: atmosphere.F90,v 20.0.2.2 2014/12/12 15:42:57 pjp Exp $'
      
character(len=128) :: tagname= &
'$Name: no_tracer_manager_pjp $'
...
call tracer_type_init(tracer_attributes)
num_tracers = size(tracer_attributes)
...
integer :: ntr
real, dimension(size(grid_tracers,1),size(grid_tracers,2),size(grid_tracers,3)) :: condense, evap_rate, rh, rh2
real, dimension(size(grid_tracers,1),size(grid_tracers,2)) :: rain, evap
...
do ntr=1,num_tracers
  if(tracer_attributes(ntr)%name == 'radon') then
    call radon_source_sink(tracer_attributes(ntr), grid_tracers(:,:,:,previous,ntr), p_half(:,:,:,current), delta_t, dt_tracers(:,:,:,ntr))
  endif
enddo
...
call complete_robert_filter ! The robert filtering is already done except in the special case where step_number == num_steps in subroutine spectral_dynamics
...
call spectral_diagnostics(Time_next, psg(:,:,future), ug(:,:,:,future), vg(:,:,:,future), &
                          tg(:,:,:,future), wg_full, grid_tracers(:,:,:,:,:), condense, evap_rate, rh, rh2, rain, evap, future)
