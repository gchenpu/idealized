...
use        tracer_type_mod, only: tracer_type, get_tracer_index , NO_TRACER
...
use tracer_source_sink_mod, only: water_vapor_source_sink

use strat_cloud_mod,            only: strat_cloud_init, strat_cloud_end

!use atmosphere_mod,     only: get_atmosphere_axes
...
character(len=128), parameter :: version = '$Id: spectral_dynamics.F90,v 19.0.2.4 2014/12/15 18:10:10 Peter.Phillipps Exp $'
character(len=128), parameter :: tagname = '$Name: no_tracer_manager_pjp $'
...
integer :: id_condense, id_evap_rate, id_rh, id_rain, id_evap, id_rh2
...
integer :: num_tracers, nsphum, nmix_rat, nql, nqi, nqa
real :: initial_atmos_water, all_rain, all_evap ! sum of global mean rain and evap over entire integration

type(time_type) :: Time_step, Alarm_time, Alarm_interval ! Used to determine when it is time to print global integrals.
type(tracer_type), allocatable :: tracer_attributes(:)
...
integer :: pe, npes, nhum, t_vert_advect_scheme, uv_vert_advect_scheme, step_number
...
integer, dimension(2) ::  print_interval=(/100,0/)
...
subroutine spectral_dynamics_init(Time, Time_step_in, tracer_attributes_in, dry_model_out, nhum_out, ocean_mask)

type(tracer_type), intent(in) :: tracer_attributes_in(:)
...
integer :: num_total_wavenumbers, unit, k, seconds, days, ierr, io, ntr
logical :: south_to_north = .true.
real    :: ref_surf_p_implicit
...
num_tracers = size(tracer_attributes_in)
allocate(tracer_attributes(num_tracers))
call allocate_fields

!tracer_attributes = tracer_attributes_in ! This doesn't always work
do ntr=1,num_tracers
   tracer_attributes(ntr)%name                     = tracer_attributes_in(ntr)%name
   tracer_attributes(ntr)%longname                 = tracer_attributes_in(ntr)%longname
   tracer_attributes(ntr)%units                    = tracer_attributes_in(ntr)%units
   tracer_attributes(ntr)%numerical_representation = tracer_attributes_in(ntr)%numerical_representation
   tracer_attributes(ntr)%advect_vert              = tracer_attributes_in(ntr)%advect_vert
   tracer_attributes(ntr)%hole_filling             = tracer_attributes_in(ntr)%hole_filling
   tracer_attributes(ntr)%robert_coeff             = tracer_attributes_in(ntr)%robert_coeff
   tracer_attributes(ntr)%flux                     = tracer_attributes_in(ntr)%flux
   tracer_attributes(ntr)%sink                     = tracer_attributes_in(ntr)%sink
enddo

nsphum   = get_tracer_index(tracer_attributes, 'sphum')
nmix_rat = get_tracer_index(tracer_attributes, 'mix_rat')
nql = get_tracer_index(tracer_attributes, 'liq_wat')
nqi = get_tracer_index(tracer_attributes, 'ice_wat')
nqa = get_tracer_index(tracer_attributes, 'cld_amt')
...
call read_restart_or_do_coldstart(ocean_mask)

call press_and_geopot_init(pk, bk, use_virtual_temperature, vert_difference_option)

call spectral_diagnostics_init(Time)

call every_step_diagnostics_init(Time, lon_max, lat_max, num_levels, reference_sea_level_press, tracer_attributes)
...
initial_atmos_water = mass_weighted_global_integral(grid_tracers(:,:,:,current,nhum), psg(:,:,current))
all_rain = 0.0
all_evap = 0.0
...
!===============================================================================================
subroutine read_restart_or_do_coldstart(ocean_mask)
...
subroutine spectral_dynamics(Time, psg_final, ug_final, vg_final, tg_final, grid_tracers_final, &
                             time_level_out, dt_psg, dt_ug, dt_vg, dt_tg, dt_tracers, wg_full, p_full, p_half, z_full, &
                             condense, evap_rate, rh, rh2, rain, evap)
...
real, intent(inout), dimension(is:, js:, :   ) :: condense, evap_rate, rh, rh2
real, intent(inout), dimension(is:, js:      ) :: rain, evap
...
! The "current" values of p_half are passed to update_tracers, but for psg the "future" values are passed.
! "current" values of pressure are used by vertical advection, "future" values by water vapor source and sink.
! Advection tendencies are used in the leapfrog time stepping scheme. Water vapor source and sink is not done by leapfrogging.
call update_tracers(Time, dt_tracers_tmp, wg, p_half, psg(:,:,future), delta_t, condense, evap_rate, rh, rh2, rain, evap)
all_rain = all_rain + dt_real*area_weighted_global_mean(rain)
all_evap = all_evap + dt_real*area_weighted_global_mean(evap)

call compute_corrections(delta_t)
...
subroutine update_tracers(Time, dt_tr, wg, p_half, psurf, delta_t, condense, evap_rate, rh, rh2, rain, evap)

type(time_type), intent(in) :: Time
real   , intent(inout), dimension(:,:,:,:) :: dt_tr
real   , intent(in   ), dimension(:,:,:  ) :: wg, p_half
real   , intent(in   ), dimension(:,:    ) :: psurf
...
real,    dimension(is:ie, js:je, num_levels) :: dp, dt_tmp, tr_future, p_full, dummy2, ql_future, qi_future, qa_future, tg2
real,    dimension(is:ie, js:je, num_levels+1) :: p_half_future, dummy1
...
!yim: where water tracer is handled
!    if(trim(tracer_attributes(ntr)%name) == 'mix_rat') then
!      call pressure_variables(p_half_future, dummy1, p_full, dummy2, psurf)
!      dp = p_half_future(:,:,2:num_levels+1) - p_half_future(:,:,1:num_levels)
!      call water_vapor_source_sink(tracer_attributes(ntr), tg(:,:,:,future), dp, p_full, delta_t, tr_future, condense, rh, rain, evap)
!    endif
...
do ntr = 1, num_tracers
    tr_future = grid_tracers(:,:,:,future,ntr)
    if(trim(tracer_attributes(ntr)%name) == 'mix_rat') then
      call pressure_variables(p_half_future, dummy1, p_full, dummy2, psurf)
      dp = p_half_future(:,:,2:num_levels+1) - p_half_future(:,:,1:num_levels)
      ql_future = grid_tracers(:,:,:,future,nql)
      qi_future = grid_tracers(:,:,:,future,nqi)
      qa_future = grid_tracers(:,:,:,future,nqa)
!yim perturbation plus 2 K
      tg2 = tg(:,:,:,future) + 6.0
      call water_vapor_source_sink(Time, is, ie, js, je, tracer_attributes(ntr), tg(:,:,:,future), dp, p_full, p_half_future, delta_t, tr_future, &
!      call water_vapor_source_sink(Time, is, ie, js, je, tracer_attributes(ntr), tg2, dp, p_full, p_half_future, delta_t, tr_future, &
      ql_future, qi_future, qa_future, condense, evap_rate, rh, rh2, rain, evap)
    endif

    if(trim(tracer_attributes(ntr)%name) == 'liq_wat') then
      tr_future = ql_future      
    endif

    if(trim(tracer_attributes(ntr)%name) == 'ice_wat') then
      tr_future = qi_future      
    endif

    if(trim(tracer_attributes(ntr)%name) == 'cld_amt') then
      tr_future = qa_future      
    endif
...
subroutine compute_corrections(delta_t)
...
!================================================================================
subroutine complete_update_of_future(psg_in, ug_in, vg_in, tg_in, grid_tracers_in)
...
!================================================================================
subroutine complete_robert_filter
integer :: ntr
...
subroutine spectral_dynamics_end(Time)

type(time_type), intent(in), optional :: Time
integer :: ntr, nt
real :: final_atmos_water
character(len=64) :: file, tr_name
character(len=42) :: rain_mesg = 'all_rain =                          Kg/m^2'
character(len=42) :: evap_mesg = 'all_evap =                          Kg/m^2'
character(len=59) :: init_mesg = 'initial atmospheric water =                          Kg/m^2'
character(len=59) :: finl_mesg = 'final   atmospheric water =                          Kg/m^2'
character(len=107) :: totl_mesg = 'all_evap - all_rain + initial atmospheric water - final atmospheric water =                          Kg/m^2'

if(.not.module_is_initialized) return

final_atmos_water = mass_weighted_global_integral(grid_tracers(:,:,:,current,nhum), psg(:,:,current))
write(rain_mesg(12:35),'(e24.16)') all_rain
write(evap_mesg(12:35),'(e24.16)') all_evap
write(init_mesg(29:52),'(e24.16)') initial_atmos_water
write(finl_mesg(29:52),'(e24.16)') final_atmos_water
write(totl_mesg(77:100),'(e24.16)') all_evap - all_rain + initial_atmos_water - final_atmos_water
call error_mesg('spectral_diagnostics_end',trim(rain_mesg),NOTE)
call error_mesg('spectral_diagnostics_end',trim(evap_mesg),NOTE)
call error_mesg('spectral_diagnostics_end',trim(init_mesg),NOTE)
call error_mesg('spectral_diagnostics_end',trim(finl_mesg),NOTE)
call error_mesg('spectral_diagnostics_end',trim(totl_mesg),NOTE)
...
if(nmix_rat /= NO_TRACER) then
   id_condense = register_diag_field(mod_name,'condense',axes_3d_full, Time,'condensed water',  '(Kg water/Kg air)/sec')
   id_evap_rate = register_diag_field(mod_name,'evap_rate',axes_3d_full, Time,'evaporated water',  '(Kg water/Kg air)/sec')
   id_rh       = register_diag_field(mod_name,'rh',      axes_3d_full, Time,'relative humidity','0 to 1')
   id_rh2      = register_diag_field(mod_name,'rh2',     axes_3d_full, Time,'relative humidity','0 to 1')
   id_rain     = register_diag_field(mod_name,'rain',(/id_lon,id_lat/),Time,'rain',             '(Kg water/m**2)/sec')
   id_evap     = register_diag_field(mod_name,'evap',(/id_lon,id_lat/),Time,'evap',             '(Kg water/m**2)/sec')
endif
...
do ntr=1,num_tracers
  id_tr(ntr) = register_diag_field(mod_name, tracer_attributes(ntr)%name, axes_3d_full, Time, &
                            tracer_attributes(ntr)%longname, tracer_attributes(ntr)%units)
enddo
...
!id_lat=js
       call strat_cloud_init (axis_id, Time, id_lon, id_lat, id_pfull,    &
                                 do_legacy_strat_cloud = do_legacy_strat_cloud)

return
end subroutine spectral_diagnostics_init
!===================================================================================
subroutine spectral_diagnostics(Time, p_surf, u_grid, v_grid, t_grid, wg_full, tr_grid, condense, evap_rate, rh, rh2, rain, evap, time_level)
...
real, intent(in), dimension(is:, js:, :)       :: condense, evap_rate, rh, rh2
real, intent(in), dimension(is:, js:)          :: rain, evap
...
if(nmix_rat /= NO_TRACER) then
   if(id_condense > 0)  used = send_data(id_condense, condense, Time)
   if(id_evap_rate > 0)  used = send_data(id_evap_rate, evap_rate, Time)
   if(id_rh > 0)        used = send_data(id_rh,   rh,   Time)
   if(id_rh2 > 0)        used = send_data(id_rh2,   rh2,   Time)
   if(id_rain > 0)      used = send_data(id_rain, rain, Time)
   if(id_evap > 0)      used = send_data(id_evap, evap, Time)
endif
