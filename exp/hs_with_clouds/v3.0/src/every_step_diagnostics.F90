...
character(len=128), parameter :: version = '$Id: every_step_diagnostics.F90,v 13.0.4.1 2014/12/04 17:56:01 Peter.Phillipps Exp $'
...
subroutine every_step_diagnostics_init(Time, lon_max, lat_max, num_levels_in, reference_sea_level_press, tracer_attributes)

type(time_type), intent(in) :: Time
integer, intent(in) :: lon_max, lat_max, num_levels_in
real, intent(in) :: reference_sea_level_press
type(tracer_type), intent(in) :: tracer_attributes(:)
...
do ntr=1,num_tracers
  id_tr(ntr) = register_diag_field(mod_name, trim(tracer_attributes(ntr)%name)//'_every', axes_3d, Time, &
                                trim(tracer_attributes(ntr)%longname), trim(tracer_attributes(ntr)%units))
enddo
...
do ntr=1,num_tracers
  tname = tracer_attributes(ntr)%name
  longname = tracer_attributes(ntr)%longname
  units = tracer_attributes(ntr)%units
  two_dt_id_tr(ntr) = &
       register_static_field(mod_name, '2dt_'//trim(tracer_attributes(ntr)%name), axes_3d, &
       'Amplitude of 2*dt wave in '//trim(tracer_attributes(ntr)%longname), trim(tracer_attributes(ntr)%units))
enddo
iwt = 1
num_time_steps = 0
