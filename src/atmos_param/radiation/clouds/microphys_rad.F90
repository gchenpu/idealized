...
!  longwave/shortwave package modules

use sealw99_mod,         only:  sealw99_number_of_bands, NBLW
...
real, parameter :: LW_BAND_RESOLUTION = 10.
...
!    emissivity. check for multi-band emissivity parameterization;
!    do_lw_micro implies a microphysically-based parameterization.
!    ebert-curry is a single band, microphysically based scheme,
!    fuliou is a multi-band microphysically based scheme.
!--------------------------------------------------------------------
...
!    is given by lw_band_resolution.
!--------------------------------------------------------------------
        iendfubands(:) = INT((endfubands(:) + 0.01)/ LW_BAND_RESOLUTION)
...
!--------------------------------------------------------------------
        do n=1,NBLW 
          centnb    = 5.0 + (n - 1)*LW_BAND_RESOLUTION
          c1        = (3.7412E-05)*centnb   **3
          x         = 1.4387E+00*centnb   /xtemv
          x1        = EXP(x   )
          sc        = c1   /(x1    - 1.0E+00)
          src1nb(n) = LW_BAND_RESOLUTION*sc
        end do
...
!--------------------------------------------------------------------
!BW   if (Cldrad_control%do_sw_micro_iz) then
!BW   else
!BW     call error_mesg ('microphys_rad_mod', &
!BW      'Cldrad_control%do_sw_micro has not been initialized', FATAL)
!BW   endif
...
!--------------------------------------------------------------------
!     number of solar bands and total wave numbers
!---------------------------------------------------------------------
...
!    parameterization.
!--------------------------------------------------------------------
        allocate ( nivl1liqcld    (num_sw_bands),    &
                   nivl1icecld    (num_sw_bands),   &
                   nivl1icesolcld (num_sw_bands),   &
                   nivl1raincld   (num_sw_bands),  &
                   nivl1snowcld   (num_sw_bands),  &
                   nivl2liqcld    (num_sw_bands),   &
                   nivl2icecld    (num_sw_bands),   &
                   nivl2icesolcld (num_sw_bands),   &
                   nivl2raincld   (num_sw_bands),   & 
                   nivl2snowcld   (num_sw_bands) ) 

        allocate ( solivlicecld   (num_sw_bands, nicecldivls),  &
                   solivlicesolcld(num_sw_bands, nicesolarcldivls), &
                   solivlliqcld   (num_sw_bands, nliqcldivls), &
                   solivlraincld  (num_sw_bands, nraincldivls), &
                   solivlsnowcld  (num_sw_bands, nsnowcldivls) )
...
!---------------------------------------------------------------------
        ...
        call esfsw_band_segments ( endliqcldwvn,    solivlliqcld,    nivl1liqcld,    nivl2liqcld )
        call esfsw_band_segments ( endicecldwvn,    solivlicecld,    nivl1icecld,    nivl2icecld )
        call esfsw_band_segments ( endraincldwvn,   solivlraincld,   nivl1raincld,   nivl2raincld )
        call esfsw_band_segments ( endsnowcldwvn,   solivlsnowcld,   nivl1snowcld,   nivl2snowcld )
        call esfsw_band_segments ( endicesolcldwvn, solivlicesolcld, nivl1icesolcld, nivl2icesolcld )

!BW   endif   ! (do_sw_micro)
...
!  </IN>
! </SUBROUTINE>
subroutine microphys_sw_driver (is, ie, js, je, &
                                Cldrad_control, Cloud_microphysics,  &
...
!  </IN>
! </SUBROUTINE>
subroutine microphys_lw_driver (is, ie, js, je, &
                                Cldrad_control, Cloud_microphysics,  &
                                Cloud_rad_props, Micro_rad_props, &
                                donner_flag )
...
!---------------------------------------------------------------------
      ...
      do nnn=1,nbprofiles ! loop over profiles
        nonly = 0
        call cloud_lwpar (nonly, nbmax, nnn, Cldrad_control%using_fu2007, &
                          size_drop, size_ice,   &
                          Cloud_microphysics%size_rain,              &
                          conc_drop, conc_ice, &
                          Cloud_microphysics%conc_rain, &
...
subroutine isccp_microphys_sw_driver (is, js, iswband, &
                                Cldrad_control, Cloud_microphysics, cldext )
...
subroutine isccp_microphys_lw_driver (is, js, ilwband, &
                                Cldrad_control, Cloud_microphysics, abscoeff )
...
!---------------------------------------------------------------------
      if (trim(lwem_form) == 'fuliou') then
        ...
        call cloud_lwpar (nonly, nbmax, 1, Cldrad_control%using_fu2007, &
                          size_drop, size_ice,   &
                          Cloud_microphysics%size_rain,              &
                          conc_drop, conc_ice, &
                          Cloud_microphysics%conc_rain, &
...
!      abscoeff  lw absorption coefficient for each of the num_lw_bands
!                bands [ km**(-1) ]
!                                                                   
!   intent(out) variables:                                     
!                                                                   
!      cldemiss  the infrared cloud emissivity for each of the num_lw_bands
!                bands [ dimensionless ]
!                                                                   
!----------------------------------------------------------------------
...
!                             Cloud_microphys, Cloudrad_props )
!  </TEMPLATE>
...
!  </INOUT>
!  <IN NAME="Cloud_microphys" TYPE="microphysics_type">
!   microphysical specification for clouds
!  </IN>
!  <IN NAME="Cloudrad_props" TYPE="microrad_properties_type">
!   cloud radiative properties for clouds
!                      clouds associated with donner convection
!  </IN>
! </SUBROUTINE>
! 

subroutine comb_cldprops_calc ( deltaz, stoch_cloud_type, &
                                cldext, cldsct, cldasymm, abscoeff, &
                                Cldrad_control, Cloud_microphys, Cloudrad_props )
...
type(microrad_properties_type), intent(in), dimension(:) :: Cloudrad_props
...
!                      each of the longwave frequency bands [ km**(-1) ]
!
!   intent(in) variables, dimensioned by the number of cloud schemes:
!
!     Cloud_microphys  microphysical specification for cloud schemes
!                      [ microphysics_type ]
!     Cloudrad_props   cloud radiative properties for cloud schemes
!                      [ microrad_properties_type ]
!
!---------------------------------------------------------------------
...
!  local variables:
      ...
      real, dimension (size(cldext,1), size(cldext,2), size(cldext,3)) :: cldsum
      real, dimension (size(cldext,1), size(cldext,2), &
                       size(cldext,3), size(Cloud_microphys(:))) :: cldamt
      real :: sumext, sumsct, sumabs, sumext2, sumsct2, sumabs2
...
!---------------------------------------------------------------------
     ...
     ncld = size(Cloud_microphys(:))
     if (ncld .ne. size(Cloudrad_props(:))) call error_mesg ('microphys_rad_mod', &
          'both Cloud_microphys and Cloudrad_props must have the same size', FATAL)

     do nc = 1, ncld
        if (.not. string_array_index( Cloud_microphys(nc)%scheme_name, Cloudrad_props(1:ncld)%scheme_name)) then
           call error_mesg ('microphys_rad_mod', &
...
!---------------------------------------------------------------------
!    number of shortwave and lonwave bands
!---------------------------------------------------------------------

        num_sw_bands = size(cldext,4)
...
!---------------------------------------------------------------------
          cldsum = 0.0
          do nc = 1, ncld
            cldamt(:,:,:,nc) = Cloud_microphys(nc)%cldamt
          enddo
          cldsum = sum(cldamt,4)
            ...
            do j=1,size(cldext,2)
              do i=1,size(cldext,1)
                if (cldsum(i,j,k) > 0.0) then
                  ...
                  sumext2 = 0.0
                  sumsct2 = 0.0
                  cldasymm(i,j,k,n,1) = 0.0
                  do nc = 1, ncld  
                     sumext = sumext + cldamt(i,j,k,nc)*Cloudrad_props(nc)%cldext(i,j,k,n)
                     sumsct = sumsct + cldamt(i,j,k,nc)*Cloudrad_props(nc)%cldsct(i,j,k,n)
                     sumext2 = sumext2 + cldamt(i,j,k,nc)*exp(-Cloudrad_props(nc)%cldext(i,j,k,n)*deltaz(i,j,k)/1000.)
                     sumsct2 = sumsct2 + cldamt(i,j,k,nc)*exp(-Cloudrad_props(nc)%cldsct(i,j,k,n)*deltaz(i,j,k)/1000.)
                     cldasymm(i,j,k,n,1) = cldasymm(i,j,k,n,1) + cldamt(i,j,k,nc)*Cloudrad_props(nc)%cldsct(i,j,k,n)* &
                                                                                  Cloudrad_props(nc)%cldasymm(i,j,k,n)
                  enddo
                  cldext(i,j,k,n,1) = min(-1000.*alog(sumext2/cldsum(i,j,k))/deltaz(i,j,k), sumext/cldsum(i,j,k))
                  cldsct(i,j,k,n,1) = min(-1000.*alog(sumsct2/cldsum(i,j,k))/deltaz(i,j,k), sumsct/cldsum(i,j,k))
                  cldasymm(i,j,k,n,1) = cldasymm(i,j,k,n,1)/sumsct
            ...
            do j=1,size(cldext,2)
              do i=1,size(cldext,1)
                if (cldsum(i,j,k) > 0.0) then
                  ...
                  sumabs2 = 0.0
                  do nc = 1, ncld  
                     sumabs = sumabs + cldamt(i,j,k,nc)*Cloudrad_props(nc)%abscoeff(i,j,k,n)
                     sumabs2 = sumabs2 + cldamt(i,j,k,nc)*exp(-Cloudrad_props(nc)%abscoeff(i,j,k,n)*deltaz(i,j,k)/1000.)
                  enddo
                  abscoeff(i,j,k,n,1) = min(-1000.*alog(sumabs2/cldsum(i,j,k))/deltaz(i,j,k), sumabs/cldsum(i,j,k))
...
!----------------------------------------------------------------------
        ...
        do n = 1, num_sw_bands
           do k = 1, size(cldext,3) ! level
           do j = 1, size(cldext,2) ! lat
           do i = 1, size(cldext,1) ! lon
              if (stoch_cloud_type(i,j,k,n) >= 1 .and. stoch_cloud_type(i,j,k,n) <= ncld) then
                 !--- assign cloud properties by index ---
                 cldext(i,j,k,n,1) = Cloudrad_props(stoch_cloud_type(i,j,k,n))%cldext(i,j,k,n)
                 cldsct(i,j,k,n,1) = Cloudrad_props(stoch_cloud_type(i,j,k,n))%cldsct(i,j,k,n)
                 cldasymm(i,j,k,n,1) = Cloudrad_props(stoch_cloud_type(i,j,k,n))%cldasymm(i,j,k,n)
              else
                 cldext(i,j,k,n,1) = 0.
                 cldsct(i,j,k,n,1) = 0.
...
!----------------------------------------------------------------------
        ...
        do n = 1, num_lw_bands
           nn = num_sw_bands+n
           do k=1,size(cldext,3) ! level
           do j=1,size(cldext,2) ! lon
           do i=1,size(cldext,1) ! lat
              if (stoch_cloud_type(i,j,k,nn) >= 1 .and. stoch_cloud_type(i,j,k,nn) <= ncld) then
                 abscoeff(i,j,k,n,1) = Cloudrad_props(stoch_cloud_type(i,j,k,nn))%abscoeff(i,j,k,n)
              else
...
!                     calculations
!
!        Note that nbmax = number of solar bands in the following cases
!               
...
!----------------------------------------------------------------------
          call esfsw_thickavg (nivl1liqcld, nivl2liqcld, NLIQCLDIVLS,     &
                         num_sw_bands, cldextivlliq,   &
...
!----------------------------------------------------------------------
          call esfsw_thickavg (nivl1raincld, nivl2raincld, NRAINCLDIVLS,  &
                     num_sw_bands, cldextivlrain,    &
...
!    interval to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
            call esfsw_thickavg (nivl1icecld, nivl2icecld, NICECLDIVLS,   &
                           num_sw_bands, cldextivlice,     &
...
!    interval to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
            call esfsw_thickavg (nivl1icesolcld, nivl2icesolcld,    &
                           NICESOLARCLDIVLS, num_sw_bands, &
                           cldextivlice2, cldssalbivlice2,  &
...
!    to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
          call esfsw_thickavg (nivl1snowcld, nivl2snowcld, NSNOWCLDIVLS,  &
                         num_sw_bands, cldextivlsnow,     &
...
!----------------------------------------------------------------------
            if (nonly == 0 ) then
              call esfsw_thickavg (nb, nivl1liqcld(nb), nivl2liqcld(nb),  &
                            NLIQCLDIVLS,   &
                        num_sw_bands, cldextivlliq,    &
                        cldssalbivlliq, cldasymmivlliq, solivlliqcld, &
                        maskl, &
...
!    to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
              call esfsw_thickavg ( nb, nivl1raincld(nb), nivl2raincld(nb), &
                              NRAINCLDIVLS,  &
                     num_sw_bands, cldextivlrain,    &
                     cldssalbivlrain , cldasymmivlrain,  &
                     solivlraincld,  &
...
!    interval to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
                call esfsw_thickavg (nb, nivl1icecld(nb), nivl2icecld(nb), &
                               NICECLDIVLS, &
                           num_sw_bands, cldextivlice,     &
                           cldssalbivlice, cldasymmivlice,     &
                           solivlicecld,   &
                ...
                call esfsw_thickavg (nb, nivl1icesolcld(nb),  &
                              nivl2icesolcld(nb),    &
                           NICESOLARCLDIVLS, num_sw_bands, &
                           cldextivlice2, cldssalbivlice2,  &
                           cldasymmivlice2,&
...
!    to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
              call esfsw_thickavg (nb, nivl1snowcld(nb), nivl2snowcld(nb), &
                            NSNOWCLDIVLS,  &
                     num_sw_bands, cldextivlsnow,     &
                     cldssalbivlsnow , cldasymmivlsnow,  &
                     solivlsnowcld,  &
...
!    interval to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
              call esfsw_thickavg (nonly, nivl1raincld(nonly),  &
                             nivl2raincld(nonly),  &
                             cldextivlrain, solivlraincld,  &
...
!    interval to the sw parameterization band spectral intervals.
!----------------------------------------------------------------------
              call esfsw_thickavg (nonly, nivl1snowcld(nonly),   &
                             nivl2snowcld(nonly), &
                                         cldextivlsnow,     &
                  solivlsnowcld,  &
...
!  </IN>
! </SUBROUTINE>
subroutine cloud_lwpar (nonly, nbmax, nnn, using_fu2007, size_drop, size_ice,  &
...
integer,                   intent(in)   ::   nonly, nbmax, nnn
logical,                   intent(in)   ::   using_fu2007
real, dimension (:,:,:,:), intent(in)   ::   size_drop, size_ice,    &
                                             conc_drop, conc_ice
logical, dimension (:,:,:,:), intent(in)   :: dge_column
real, dimension (:,:,:),   intent(in)   ::   size_rain, conc_rain,   &
...
!                     calculations
!
!        Note that nbmax = num_lw_bands in the following cases
!               
...
!----------------------------------------------------------------------
      if (isccp_call) then
        do n=1,num_lw_bands
          if (nbmax == 1) then
            call el_dge (n, using_fu2007, conc_ice(:,:,:,nnn), size_ice(:,:,:,nnn),  &
                          
                  dge_column(:,:,:,nnn), maskf, cldext, cldssa, cldasy)
           else
             if (nonly.eq.0   .or. nonly.eq.n ) then            
             call el_dge (n, using_fu2007, conc_ice(:,:,:,n), size_ice(:,:,:,n),  &
...
subroutine el_dge (nb, using_fu2007, conc_ice, size_ice, dge_column, mask, &
                   cldextbndicelw, cldssalbbndicelw, cldasymmbndicelw)
      ...
      do k=1, size(conc_ice,3)
        do j=1, size(conc_ice,2)
          do i=1, size(conc_ice,1)
            ...
            !BW if ((Cldrad_control%using_fu2007 .and.    &
                if ((using_fu2007 .and.    &
                    size_i(i,j,k) > 15.0) .or. &
                    .not. using_fu2007) then
